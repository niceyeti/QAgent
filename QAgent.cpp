...
QAgent::QAgent(int initX, int initY)
{
	...
	for(int i = 0; i < _stateHistory.size(); i++){
		...
		for(int j = 0; j < _stateHistory[i].size(); j++){
			_stateHistory[i][j].resize(STATE_DIMENSION);
			for(int k = 0; k < _stateHistory[i][j].size(); k++){
				_stateHistory[i][j][k] = 0.0;
			}
		}
	}
...
void QAgent::_deriveCurrentState(const World* world, const vector<Missile>& missiles, vector<double>& state)
{
	...
	double x_prime = world->GOAL_X - agent.x;
	double y_prime = world->GOAL_Y - agent.y;
	double goalDist = _dist(world->GOAL_X, world->GOAL_Y, agent.x, agent.y);

	//set all the action-unique state values (the state estimate, given an action)
	for(int i = 0; i < _stateHistory[_t].size(); i++){
		//all nets share the same dist value
		_stateHistory[_t][i][SA_GOAL_DIST] = goalDist;
	
		//set the velocity/heading specific state values
		switch(i){
			case ACTION_UP:
				xHeading = 0;
				yHeading = 1.0;
		...
		_stateHistory[_t][i][SA_COLLISION_PROXIMITY] = _nearestObjectOnHeading(xHeading, yHeading, world, missiles);
		...
		if(world->GetCell(agent.x,agent.y).isGoal){
			_stateHistory[_t][i][SA_GOAL_COSINE] = 1.0;
		}
		else{
			//note this is only a measure of direction, not magnitude
			_stateHistory[_t][i][SA_GOAL_COSINE] = _cosSim(x_prime, y_prime, xHeading, yHeading);
		}
	}
...
double QAgent::_nearestObjectOnHeading(double headingX, double headingY, const World* world, const vector<Missile>& missiles)
{
	...
	for(int t = 0; t < 15; t++){
		...
		if(!world->IsValidPosition(x_t, y_t) || world->GetCell(x_t, y_t).isObstacle){
			return _dist(agent.x, agent.y, (double)x_t, (double)y_t);
		}
	}
